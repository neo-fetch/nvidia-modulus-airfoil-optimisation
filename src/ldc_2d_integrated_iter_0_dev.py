...
class PotentialSolver(Solver, PDES):
    ...
    def custom_loss(self, domain_invar, pred_domain_outvar, true_domain_outvar, step):
        ...
        for i in range(len(neighbors[0])):
            ...
            # xyff
            x_xyff = [x for x,y in neighbors[i][3]] # We create a list of the x values of the neighbors of xyff.
            y_xyff = [y for x,y in neighbors[i][3]] # We create a list of the y values of the neighbors of xyff.

            xy = neighbors[i][4] # The original point.
            ...
            # phi xyff
            phi_xyff = self.nets[0].evaluate({'x': x_xyff, 'y': y_xyff})['phi'] # We evaluate the phi values of the neighbors of xyff using the neural network.
            ...
            phi_xyff = phi_interpolation(phi_xyff, len(phi_xyff), weights[i][3]) # We interpolate the phi values of the neighbors of xyff using the weighted average.
            ...
            # Calculating the gradient of the squared phi values
            del_2_phi_x = (phi_xfy - 2*phi_xy + phi_xby)/(dx**2) # We calculate the gradient of the squared phi values of the neighbors of xfy using central difference formula.
            del_2_phi_y = [(phi_xyff - 2*phi_xyf + phi_xy)/(dy**2), (phi_xy - 2*phi_xyf + phi_xyff)/(dy**2)][xy[1] < 0] # We calculate the gradient of the squared phi values of the neighbors of xyf using central difference formula.
            grad_sq_phi.append([xy[0], xy[1]], del_2_phi_x + del_2_phi_y) # We append the gradient of the squared phi values to the list.

                                                                        # the neighbors of xfy, xby, xyf, xyff, and xy to the phi_band list.
        ...
        del_2_phi_outside_x = (phi_outside.diff('x')).diff('x')
        del_2_phi_outside_y = (phi_outside.diff('y')).diff('y')
        grad_sq_phi_outside = del_2_phi_outside_x + del_2_phi_outside_y
        ...
        grad_sq_total = grad_sq_phi + grad_sq_phi_outside
        u_total = u_band + u_outside
        v_total = v_band + v_outside

        return [u_total, v_total, grad_sq_total]
