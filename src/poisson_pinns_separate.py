...
class PotentialTrain(TrainDomain):
    def __init__(self, **config):
        ...
        end_points_negative = negative.boundary_bc(
            outvar_sympy={"f": 0}, # Mimicing the far field conditions "u":u_x , "v": u_y,
            batch_size_per_area=200,
            lambda_sympy={"lambda_f": 1}, # Weights for the loss function.
        )
        self.add(end_points_negative, name="end_points_negative")

        end_points_positive = positive.boundary_bc(
            outvar_sympy={"f": 0}, # Mimicing the far field conditions "u":u_x , "v": u_y,
            batch_size_per_area=200,
            lambda_sympy={"lambda_f": 1}, # Weights for the loss function.
        )
        self.add(end_points_positive, name="end_points_positive")
